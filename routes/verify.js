...
module.exports = function (app) {
	app.get(('/verify/:id'), async (req, res, next) => {
		try {
			...
			res.render('verify_waiting', { title: 'Velkommen', email: user[0].email });
		} catch (error) {
			next(error);
		}
	});
	...
	app.get('/verify/:id/:token', (req, res, next) => {
		jwt.verify(req.params.token, process.env.TOKEN_SECRET, async (err, decoded) => {
			...
			try {
				const user = await User.findOne({ id: req.params.id, email: decoded.data, active: 0 });

				if (user.length !== 1) return res.render('verify_error', { title: 'Fejl' });

				const result = await User.findByIdAndUpdate(user[0].id, { active: 1 });
