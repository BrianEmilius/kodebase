...
module.exports = function (app) {
	...
	app.post('/signup', async (req, res, next) => {
		try {
			...
			const token = await jwt.sign({ data: req.fields.email },
				process.env.TOKEN_SECRET,
				{ expiresIn: '1d' });
			const activationMail = new Mail('Please verify your email address',
			`<p><a href="http://localhost:3030/verify/${user.insertId}/${token}">Click here to verify</a></p>`);
